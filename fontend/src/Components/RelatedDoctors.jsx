    ...
    <div className='flex flex-col items-center gap-4 my-16 text-blue-600 md:mx-10'>
      ...
      <div className='w-full grid grid-cols-4 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
          {relDoc.slice(0,5).map((item,index)=>(
            <div key={index} onClick={()=>{navigate(`/appointments/${item._id}`); scrollTo(0,0) }} className='border border-gray-200 rounded-xl'>
              <img className='bg-blue-50 cursor-pointer rounded-xl' src={item.image} alt="" />
                <div className="p-4 text-left text-gray-900">
